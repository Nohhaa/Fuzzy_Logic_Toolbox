...
struct FuzzyRules
{
    ...
    // Constructor
    FuzzyRules() {}
    FuzzyRules(const string &var1, const string &s1, const string &operatorType,
               const string &var2, const string &s2, const string &outVar, const string &outS)
        : variable1(var1), set1(s1), op(operatorType), variable2(var2), set2(s2), outVariable(outVar), outSet(outS) {}

    void displayInfo() const
    {
        cout << "Fuzzy Rule: " << variable1 << " " << set1 << " " << op << " "
             << variable2 << " " << set2 << " -> " << outVariable << " " << outSet << endl;
    }
};
struct FuzzySet
{
